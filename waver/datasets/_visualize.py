...
def run_and_visualize(**kawrgs):
    ...
    wave_dict = {'colormap': wave_cmap, 'contrast_limits':[-clim, clim], 'name': 'wave', 'metadata':kawrgs}

    speed_cmap = Colormap([[0, 0, 0, 0], [0.7, 0.5, 0, 1]], name='Orange')
    speed_dict = {'colormap': speed_cmap, 'opacity': 0.5,
                'name': 'speed', 'metadata':kawrgs}
    
    viewer = napari.Viewer()
    viewer.add_image(wave, **wave_dict)
    viewer.add_image(speed[0], **speed_dict)
